...
Trains both models if they don't exist.
"""
import os
import sys
import subprocess
...
def train_model_if_not_exists():
    """Train both models if they don't exist"""
    ...
    # ============================================
    # 1. Train GREYC Model (P5)
    # ============================================
    greyc_model_path = models_dir / "typeprint_p5_model.pkl"
    
    if greyc_model_path.exists():
        logger.info("✅ GREYC model already exists. Skipping training.")
    else:
        ...
        if not data_path.exists():
            logger.error("❌ Dataset not found at data/GREYC-NISLABKeystrokeBenchmarkDatasetSyed.xlsx")
            logger.error("   Please add the dataset file to backend/data/")
        else:
            try:
                # Run train.py
    ...
    # ============================================
    # 2. Train Browser Model (if data exists)
    # ============================================
    ...
    if browser_model_path.exists():
        logger.info("✅ Browser model already exists. Skipping training.")
    else:
        ...
        if browser_data_path.exists():
            ...
            logger.info("   (Data will be collected when users type with consent)")
    ...
    if greyc_model_path.exists():
        logger.info(f"✅ GREYC model: {greyc_model_path}")
    else:
        logger.info("❌ GREYC model: MISSING")
    
    if browser_model_path.exists():
        logger.info(f"✅ Browser model: {browser_model_path}")
    else:
